...
const repairTrainingSettingsCandidate = (value: unknown): Record<string, unknown> => {
  ...
  if (isFiniteNumber(repaired['chaseStartFallMs']) && isFiniteNumber(repaired['chaseMinFallMs'])) {
...
const normalizeCustomSequence = (
  customSequence: unknown,
...
export const normalizeTrainingSettings = (
  ...
  const normalizedMixedLettersPercent =
    typeof candidate['mixedLettersPercent'] === 'number'
      ? Math.min(
          Math.max(Math.trunc(candidate['mixedLettersPercent']), MIXED_LETTERS_PERCENT_MIN),
          MIXED_LETTERS_PERCENT_MAX,
        )
  ...
  const merged = {
    ...
    qsbEnabled:
      typeof candidate['qsbEnabled'] === 'boolean' ? candidate['qsbEnabled'] : fallback.qsbEnabled,
    ...
    qrnEnabled:
      typeof candidate['qrnEnabled'] === 'boolean' ? candidate['qrnEnabled'] : fallback.qrnEnabled,
    ...
    qrmEnabled:
      typeof candidate['qrmEnabled'] === 'boolean' ? candidate['qrmEnabled'] : fallback.qrmEnabled,
    ...
    chaseLives: Math.trunc(
      normalizeFiniteNumber(
        candidate['chaseLives'],
        fallback.chaseLives,
        CHASE_LIVES_MIN,
        CHASE_LIVES_MAX,
      ),
    ),
    chaseAutoLevelEnabled:
      typeof candidate['chaseAutoLevelEnabled'] === 'boolean'
        ? candidate['chaseAutoLevelEnabled']
        : fallback.chaseAutoLevelEnabled,
    chaseGroupsPerLevel: Math.trunc(
      normalizeFiniteNumber(
        candidate['chaseGroupsPerLevel'],
        fallback.chaseGroupsPerLevel,
        CHASE_GROUPS_PER_LEVEL_MIN,
        CHASE_GROUPS_PER_LEVEL_MAX,
      ),
    ),
    chaseStartFallMs: Math.trunc(
      normalizeFiniteNumber(
        candidate['chaseStartFallMs'],
        fallback.chaseStartFallMs,
        CHASE_FALL_MS_MIN,
        CHASE_FALL_MS_MAX,
      ),
    ),
    chaseMinFallMs: Math.trunc(
      normalizeFiniteNumber(
        candidate['chaseMinFallMs'],
        fallback.chaseMinFallMs,
        CHASE_FALL_MS_MIN,
        CHASE_FALL_MS_MAX,
      ),
    ),
    chaseLevelSpeedupMs: Math.trunc(
      normalizeFiniteNumber(
        candidate['chaseLevelSpeedupMs'],
        fallback.chaseLevelSpeedupMs,
        CHASE_SPEEDUP_MS_MIN,
        CHASE_SPEEDUP_MS_MAX,
      ),
    ),
    chaseGroupSpeedupMs: Math.trunc(
      normalizeFiniteNumber(
        candidate['chaseGroupSpeedupMs'],
        fallback.chaseGroupSpeedupMs,
        CHASE_SPEEDUP_MS_MIN,
        CHASE_SPEEDUP_MS_MAX,
      ),
    ),
  ...
  if (typeof candidate['charWpmMin'] === 'number')
    partialResult.charWpmMin = candidate['charWpmMin'];
  if (typeof candidate['charWpmMax'] === 'number')
    partialResult.charWpmMax = candidate['charWpmMax'];
  if (typeof candidate['effectiveWpmMin'] === 'number')
    partialResult.effectiveWpmMin = candidate['effectiveWpmMin'];
  if (typeof candidate['effectiveWpmMax'] === 'number')
    partialResult.effectiveWpmMax = candidate['effectiveWpmMax'];
  if (typeof candidate['numGroups'] === 'number') partialResult.numGroups = candidate['numGroups'];
  if (typeof candidate['sessionDuration'] === 'number')
    partialResult.sessionDuration = candidate['sessionDuration'];
  if (typeof candidate['charsPerGroup'] === 'number')
    partialResult.charsPerGroup = candidate['charsPerGroup'];
  if (typeof candidate['groupTimeout'] === 'number')
    partialResult.groupTimeout = candidate['groupTimeout'];
  if (typeof candidate['minGroupSize'] === 'number')
    partialResult.minGroupSize = candidate['minGroupSize'];
  if (typeof candidate['maxGroupSize'] === 'number')
    partialResult.maxGroupSize = candidate['maxGroupSize'];
  if (typeof candidate['sideToneMin'] === 'number')
    partialResult.sideToneMin = candidate['sideToneMin'];
  if (typeof candidate['sideToneMax'] === 'number')
    partialResult.sideToneMax = candidate['sideToneMax'];
  if (typeof candidate['volumeMin'] === 'number')
    partialResult.volumeMin = Math.max(0.1, Math.min(1, candidate['volumeMin']));
  if (typeof candidate['volumeMax'] === 'number')
    partialResult.volumeMax = Math.max(0.1, Math.min(1, candidate['volumeMax']));
  if (typeof candidate['linkVolume'] === 'boolean')
    partialResult.linkVolume = candidate['linkVolume'];
  if (typeof candidate['steepness'] === 'number') partialResult.steepness = candidate['steepness'];
  if (typeof candidate['extraWordSpaceMultiplier'] === 'number')
    partialResult.extraWordSpaceMultiplier = candidate['extraWordSpaceMultiplier'];
  if (typeof candidate['envelopeSmoothing'] === 'number')
    partialResult.envelopeSmoothing = candidate['envelopeSmoothing'];
  if (typeof candidate['qsbEnabled'] === 'boolean')
    partialResult.qsbEnabled = candidate['qsbEnabled'];
  if (isFiniteNumber(candidate['qsbDepth'])) {
    partialResult.qsbDepth = clampNumber(
      candidate['qsbDepth'],
      AUDIO_REALISM_LEVEL_MIN,
      AUDIO_REALISM_LEVEL_MAX,
    );
  ...
  if (typeof candidate['qrnEnabled'] === 'boolean')
    partialResult.qrnEnabled = candidate['qrnEnabled'];
  if (isFiniteNumber(candidate['qrnLevel'])) {
    partialResult.qrnLevel = clampNumber(
      candidate['qrnLevel'],
      AUDIO_REALISM_LEVEL_MIN,
      AUDIO_REALISM_LEVEL_MAX,
    );
  }
  if (typeof candidate['qrmEnabled'] === 'boolean')
    partialResult.qrmEnabled = candidate['qrmEnabled'];
  if (isFiniteNumber(candidate['qrmLevel'])) {
    partialResult.qrmLevel = clampNumber(
      candidate['qrmLevel'],
      AUDIO_REALISM_LEVEL_MIN,
      AUDIO_REALISM_LEVEL_MAX,
    );
  ...
  if (isFiniteNumber(candidate['receiverBackgroundGain'])) {
    partialResult.receiverBackgroundGain = clampNumber(
      candidate['receiverBackgroundGain'],
      RECEIVER_GAIN_MIN,
      RECEIVER_GAIN_MAX,
    );
  ...
  if (isFiniteNumber(candidate['receiverBackgroundDecay'])) {
    partialResult.receiverBackgroundDecay = clampNumber(
      candidate['receiverBackgroundDecay'],
      RECEIVER_DECAY_MIN,
      RECEIVER_DECAY_MAX,
    );
  ...
  if (isFiniteNumber(candidate['receiverBackgroundOffsetHz'])) {
    partialResult.receiverBackgroundOffsetHz = clampNumber(
      candidate['receiverBackgroundOffsetHz'],
      RECEIVER_OFFSET_MIN,
      RECEIVER_OFFSET_MAX,
    );
  ...
  if (typeof candidate['autoAdjustThreshold'] === 'number')
    partialResult.autoAdjustThreshold = candidate['autoAdjustThreshold'];
  if (typeof candidate['autoAdjustBelowThresholdCount'] === 'number')
    partialResult.autoAdjustBelowThresholdCount = candidate['autoAdjustBelowThresholdCount'];
  if (typeof candidate['autoAdjustAboveThresholdCount'] === 'number')
    partialResult.autoAdjustAboveThresholdCount = candidate['autoAdjustAboveThresholdCount'];
  if (typeof candidate['echoAutoAdjustThreshold'] === 'number')
    partialResult.echoAutoAdjustThreshold = candidate['echoAutoAdjustThreshold'];
  if (typeof candidate['echoAutoAdjustBelowThresholdCount'] === 'number') {
    partialResult.echoAutoAdjustBelowThresholdCount =
      candidate['echoAutoAdjustBelowThresholdCount'];
  }
  if (typeof candidate['echoAutoAdjustAboveThresholdCount'] === 'number') {
    partialResult.echoAutoAdjustAboveThresholdCount =
      candidate['echoAutoAdjustAboveThresholdCount'];
  }
  if (isFiniteNumber(candidate['chaseLives'])) {
    partialResult.chaseLives = Math.trunc(
      clampNumber(candidate['chaseLives'], CHASE_LIVES_MIN, CHASE_LIVES_MAX),
    );
  }
  partialResult.chaseAutoLevelEnabled =
    typeof candidate['chaseAutoLevelEnabled'] === 'boolean'
      ? candidate['chaseAutoLevelEnabled']
      : fallback.chaseAutoLevelEnabled;
  if (isFiniteNumber(candidate['chaseGroupsPerLevel'])) {
    partialResult.chaseGroupsPerLevel = Math.trunc(
      clampNumber(
        candidate['chaseGroupsPerLevel'],
        CHASE_GROUPS_PER_LEVEL_MIN,
        CHASE_GROUPS_PER_LEVEL_MAX,
  ...
  if (isFiniteNumber(candidate['chaseStartFallMs'])) {
    partialResult.chaseStartFallMs = Math.trunc(
      clampNumber(candidate['chaseStartFallMs'], CHASE_FALL_MS_MIN, CHASE_FALL_MS_MAX),
    );
  }
  if (isFiniteNumber(candidate['chaseMinFallMs'])) {
    partialResult.chaseMinFallMs = Math.trunc(
      clampNumber(candidate['chaseMinFallMs'], CHASE_FALL_MS_MIN, CHASE_FALL_MS_MAX),
    );
  }
  if (isFiniteNumber(candidate['chaseLevelSpeedupMs'])) {
    partialResult.chaseLevelSpeedupMs = Math.trunc(
      clampNumber(candidate['chaseLevelSpeedupMs'], CHASE_SPEEDUP_MS_MIN, CHASE_SPEEDUP_MS_MAX),
    );
  }
  if (isFiniteNumber(candidate['chaseGroupSpeedupMs'])) {
    partialResult.chaseGroupSpeedupMs = Math.trunc(
      clampNumber(candidate['chaseGroupSpeedupMs'], CHASE_SPEEDUP_MS_MIN, CHASE_SPEEDUP_MS_MAX),
    );
  }
  ...
  if (typeof candidate['mixedLettersPercent'] === 'number') {
    partialResult.mixedLettersPercent = Math.min(
      MIXED_LETTERS_PERCENT_MAX,
      Math.max(MIXED_LETTERS_PERCENT_MIN, Math.trunc(candidate['mixedLettersPercent'])),
    );
  } else if ((fallback as { mixedLettersPercent?: number }).mixedLettersPercent !== undefined) {
    partialResult.mixedLettersPercent = (
  ...
  partialResult.echoAutoAdjustKoch =
    typeof candidate['echoAutoAdjustKoch'] === 'boolean'
      ? candidate['echoAutoAdjustKoch']
      : fallback.echoAutoAdjustKoch;
  partialResult.linkCharWpm =
    typeof candidate['linkCharWpm'] === 'boolean' ? candidate['linkCharWpm'] : fallback.linkCharWpm;
  partialResult.linkEffectiveWpm =
    typeof candidate['linkEffectiveWpm'] === 'boolean'
      ? candidate['linkEffectiveWpm']
      : fallback.linkEffectiveWpm;
  partialResult.linkGroupSize =
    typeof candidate['linkGroupSize'] === 'boolean'
      ? candidate['linkGroupSize']
      : fallback.linkGroupSize;
  ...
  if (typeof candidate['slidingWindowStart'] === 'number') {
    partialResult.slidingWindowStart = Math.min(
      SLIDING_WINDOW_INDEX_MAX,
      Math.max(SLIDING_WINDOW_INDEX_MIN, Math.trunc(candidate['slidingWindowStart'])),
    );
  }
  if (typeof candidate['slidingWindowEnd'] === 'number') {
    partialResult.slidingWindowEnd = Math.min(
      SLIDING_WINDOW_INDEX_MAX,
  ...
  const partialParseResult = trainingSettingsSchema.safeParse(
    repairTrainingSettingsCandidate(partialResult),
  );
  ...
  console.error(
    '[settings] Even partial normalization failed, using fallback. This indicates a serious data corruption issue.',
  );
  return fallback;
};
