...
export function SequenceEditorModal({
  ...
  const [selectedPreset, setSelectedPreset] = useState<string>('lcwo');
  const [isCustom, setIsCustom] = useState(false);
  const [originalPresetId, setOriginalPresetId] = useState<string>('lcwo');
  ...
  useEffect(() => {
    if (open) {
      ...
      setOriginalPresetId(matchingPreset?.id || 'lcwo');
    }
  }, [open, sequence]);
  ...
  const handleReset = useCallback(() => {
    // Reset to the original preset that was selected when editor opened
    const preset = getPresetById(originalPresetId);
    if (preset) {
      onChange([...preset.sequence]);
      setSelectedPreset(originalPresetId);
      setIsCustom(false);
    }
      ...
      >
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between border-b pb-4">
            <h2 className="text-xl font-semibold text-gray-800">Edit Character Sequence</h2>
            <button
              type="button"
              onClick={onClose}
          ...
          <div>
            ...
            {isCustom && (
              <div className="mt-2 flex items-center gap-2">
                ...
                >
                  ↺ Reset to {getPresetById(originalPresetId)?.name || 'Preset'}
                </button>
                <span className="text-xs text-amber-600">✨ Custom sequence active</span>
              </div>
            )}
          </div>
          ...
          {/* Available Characters to Add - 5 per row */}
