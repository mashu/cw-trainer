    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 sm:mb-3 gap-2">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`text-xs sm:text-sm font-mono px-2 sm:px-3 py-1 rounded-lg border ${
              confirmed
                ? groupCorrect
          ...
          {groupCorrect && <span className="text-emerald-600 text-sm sm:text-lg">✓</span>}
        </div>
      </div>
      <input
        ref={inputRef}
            ...
            <div className="font-mono text-xs whitespace-nowrap">
              <div className="flex gap-1">
                {Array.from({ length: maxLen }).map((_, i) => (
                  <span key={`s-${i}`} className="px-1 text-slate-500">
                    {sentChars[i] || '·'}
                  </span>
                ))}
              </div>
