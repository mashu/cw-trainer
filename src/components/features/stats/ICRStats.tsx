...
export function ICRStats({ onBack, embedded }: ICRStatsProps): JSX.Element {
  ...
  const getBarFill = (ms: number, greenMax: number, yellowMax: number, orangeMax: number): string => {
    if (ms === undefined || ms === null) return '#60a5fa';
    if (ms <= greenMax) return '#10b981';
    if (ms <= yellowMax) return '#84cc16'; // lime: blend green–yellow (ICR level)
    if (ms <= orangeMax) return '#f97316';
    return '#ef4444';
  };
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-sm text-slate-600 font-medium">
                  ...
                  <span className="text-xs text-slate-500 ml-2">
                    ({hasMounted ? new Date(selectedSession.timestamp).toLocaleString() : new Date(selectedSession.timestamp).toISOString()})
                  </span>
                </div>
                <button
            ...
            {chartData.bars.length > 0 && (
              <>
                ...
                <div className="p-4 rounded-xl border border-slate-200 bg-white">
                  ...
                  {((): JSX.Element => {
                    const g = selectedSession.settingsSnapshot.icr.bucketGreenMaxMs ?? 400;
                    const y = selectedSession.settingsSnapshot.icr.bucketYellowMaxMs ?? 600;
                    const o = selectedSession.settingsSnapshot.icr.bucketOrangeMaxMs ?? 800;
                    return (
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-3 text-xs text-slate-600">
                        <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-emerald-500" aria-hidden />Green &lt;{g} ms</span>
                        <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-lime-500" aria-hidden />Yellow (ICR) &lt;{y} ms</span>
                        <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-orange-500" aria-hidden />Orange &lt;{o} ms</span>
                        <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-rose-500" aria-hidden />Red &gt;{o} ms</span>
                      </div>
                    );
                  })()}
                      ...
                      >
                        ...
                        >
                          {chartData.bars.map((entry, idx) => {
                            const greenMax =
                              selectedSession.settingsSnapshot.icr.bucketGreenMaxMs ?? 400;
                            const yellowMax =
                              selectedSession.settingsSnapshot.icr.bucketYellowMaxMs ?? 600;
                            const orangeMax =
                              selectedSession.settingsSnapshot.icr.bucketOrangeMaxMs ?? 800;
                            return (
                              <Cell
                                key={`cell-${idx}`}
                                fill={getBarFill(entry.adjAvg, greenMax, yellowMax, orangeMax)}
                              />
                            );
                          })}
                        </Bar>
