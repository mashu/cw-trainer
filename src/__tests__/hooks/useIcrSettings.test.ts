...
describe('useIcrSettings', () => {
  ...
  it('should normalize invalid values from localStorage', () => {
    ...
    expect(result.current.icrSettings.bucketOrangeMaxMs).toBeGreaterThanOrEqual(
      result.current.icrSettings.bucketYellowMaxMs,
    );
  });

  it('should clamp bucketOrangeMaxMs to be at least bucketYellowMaxMs', () => {
    const invalidSettings = {
      ...DEFAULT_ICR_SETTINGS,
      bucketGreenMaxMs: 400,
      bucketYellowMaxMs: 600,
      bucketOrangeMaxMs: 500, // Less than yellow; should be clamped to 600
    };
    ...
    expect(result.current.icrSettings.bucketOrangeMaxMs).toBe(600);
  });
