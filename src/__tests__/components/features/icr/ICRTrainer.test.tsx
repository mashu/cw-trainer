...
jest.mock('@/lib/morseAudio', () => ({
  ...
  playMorseCode: jest.fn(() => Promise.resolve({ durationSec: 1.0, startTime: 0 })),
}));
...
describe('ICRTrainer', (): void => {
  ...
  it('should enable start button when not running', async (): Promise<void> => {
    await act(async () => {
      render(
        <TestWrapper>
          <ICRTrainer sharedAudio={defaultSharedAudio} icrSettings={defaultIcrSettings} />
  ...
  it('should disable start button when running', async (): Promise<void> => {
    const user = userEvent.setup();
    await act(async () => {
      render(
        <TestWrapper>
          <ICRTrainer sharedAudio={defaultSharedAudio} icrSettings={{ ...defaultIcrSettings, trialsPerSession: 1 }} />
        </TestWrapper>,
      );
    });
    ...
    await waitFor(async () => {
      ...
      if (startButton) {
        ...
        await waitFor(() => {
          const updatedStartButton = screen.queryByRole('button', { name: /Start/i });
          expect(updatedStartButton).toBeDisabled();
        }, { timeout: 1000 });
      }
    });
  });

  it('should call stop when stop button is clicked', async (): Promise<void> => {
    const user = userEvent.setup();
    await act(async () => {
      render(
        <TestWrapper>
  ...
  it('should show session complete message when session ends', async (): Promise<void> => {
    await act(async () => {
      render(
        <TestWrapper>
          <ICRTrainer sharedAudio={defaultSharedAudio} icrSettings={{ ...defaultIcrSettings, trialsPerSession: 1 }} />
    ...
    // After a session completes, should show completion message
  ...
  it('should display average reaction and accuracy in summary', async (): Promise<void> => {
    await act(async () => {
      render(
        <TestWrapper>
          <ICRTrainer sharedAudio={defaultSharedAudio} icrSettings={defaultIcrSettings} />
    ...
    await waitFor(() => {
      const summary = screen.queryByText(/Summary/i);
      expect(summary).toBeInTheDocument();
      // Summary should contain average reaction and accuracy labels
      const avgReaction = screen.queryByText(/Average Reaction/i);
      const accuracy = screen.queryByText(/Accuracy/i);
      expect(avgReaction || accuracy).toBeTruthy();
    });
  });
  ...
  it('should handle back from stats view', async (): Promise<void> => {
    ...
    await waitFor(async () => {
      ...
      if (statsButton) {
        ...
        await waitFor(() => {
          expect(screen.queryByTestId('icr-stats')).not.toBeInTheDocument();
        });
      }
    });
  });

  it('should display trial information correctly', async (): Promise<void> => {
    await act(async () => {
      render(
        <TestWrapper>
          <ICRTrainer sharedAudio={defaultSharedAudio} icrSettings={defaultIcrSettings} />
  ...
  it('should handle different bucket settings', async (): Promise<void> => {
    const customIcrSettings: IcrSettings = {
      ...defaultIcrSettings,
      bucketGreenMaxMs: 300,
      bucketYellowMaxMs: 800,
      bucketOrangeMaxMs: 1000,
    };

    await act(async () => {
      render(
        <TestWrapper>
          <ICRTrainer sharedAudio={defaultSharedAudio} icrSettings={customIcrSettings} />
        </TestWrapper>,
      );
    });
  ...
  it('should handle VAD disabled settings', async (): Promise<void> => {
    const vadDisabledSettings: IcrSettings = {
      ...defaultIcrSettings,
      vadEnabled: false,
    };
    ...
    await act(async () => {
      render(
        <TestWrapper>
          <ICRTrainer sharedAudio={defaultSharedAudio} icrSettings={vadDisabledSettings} />
        </TestWrapper>,
      );
    });
