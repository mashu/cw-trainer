...
import { SequenceEditor } from '@/components/ui/forms/SequenceEditor';
...
describe('SequenceEditor', () => {
  ...
  it('should render with default sequence', () => {
    render(<SequenceEditor sequence={defaultSequence} onChange={mockOnChange} />);

    expect(screen.getByText(/Preset Sequence/i)).toBeInTheDocument();
    expect(screen.getByText(/Character Sequence/i)).toBeInTheDocument();
  ...
  it('should show empty state when sequence is empty', () => {
    render(<SequenceEditor sequence={[]} onChange={mockOnChange} />);
  ...
  it('should call onChange when preset is selected', async () => {
    ...
    await user.selectOptions(select, 'morsemania');
  ...
  it('should show custom option when sequence is modified', () => {
    ...
    render(<SequenceEditor sequence={customSequence} onChange={mockOnChange} />);
  ...
  it('should call onChange when character is added', async () => {
    ...
    const addButton = screen.getByTitle(/^A \(/i);
    if (addButton) {
  ...
  it('should display level slider when maxLevel and onLevelChange are provided', () => {
  ...
  it('should not display level slider when maxLevel is not provided', () => {
    render(<SequenceEditor sequence={defaultSequence} onChange={mockOnChange} />);

    expect(screen.queryByRole('slider')).not.toBeInTheDocument();
  });

  it('should call onLevelChange when slider is moved', () => {
    const mockOnLevelChange = jest.fn();
    render(
      <SequenceEditor
        sequence={defaultSequence}
  ...
  it('should show only characters up to maxLevel when maxLevel is provided', () => {
    render(
      <SequenceEditor
        sequence={defaultSequence}
        onChange={mockOnChange}
    ...
    // Should show only first 2 characters
    expect(screen.getByText(/Character Sequence \(2 characters\)/i)).toBeInTheDocument();
    expect(screen.getByText('K')).toBeInTheDocument();
    expect(screen.getByText('M')).toBeInTheDocument();
    // R should not be visible in the sequence display (but might be in preview)
  });
