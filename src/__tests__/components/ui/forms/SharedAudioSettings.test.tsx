...
describe('SharedAudioSettings', (): void => {
  ...
  it('should render form with all settings', (): void => {
    ...
    expect(screen.getAllByText(/Effective Speed/i).length).toBeGreaterThan(0);
  });
  ...
  it('should call setSettings when character WPM changes', async (): Promise<void> => {
    ...
    const charWpmLabel = screen.getAllByText(/Character Speed/i)[0]!;
  ...
  it('should sync effectiveWpm with charWpm when linkCharWpm is true', async (): Promise<void> => {
    const user = userEvent.setup();
    const setSettings = jest.fn();
  ...
  it('should allow independent effectiveWpm when linkCharWpm is false', async (): Promise<void> => {
    const user = userEvent.setup();
    const setSettings = jest.fn();
