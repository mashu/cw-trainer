...
describe('SequenceEditorModal', () => {
  ...
  it('should call onChange when preset is selected', async () => {
    ...
    const lcwoPreset = SEQUENCE_PRESETS.find((p) => p.id === 'lcwo');
    if (lcwoPreset) {
      await user.selectOptions(select, 'lcwo');
    ...
    if (lcwoPreset) {
      render(
        <SequenceEditorModal
          open={true}
          onClose={mockOnClose}
          sequence={lcwoPreset.sequence}
          onChange={mockOnChange}
        />
      );

      const select = screen.getByRole('combobox');
      expect(select).toHaveValue('lcwo');
  ...
  it('should update preset selection when sequence changes', async () => {
    ...
    if (lcwoPreset) {
      ...
      // Initially should show lcwo preset
      const select = screen.getByRole('combobox');
      expect(select).toHaveValue('lcwo');
