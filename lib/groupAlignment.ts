...
/**
 * Calculate overall character-level accuracy using alignment.
 * This counts individual characters across all groups, not groups as units.
 * 
 * @param groups Array of { sent: string, received: string }
 * @returns Accuracy score (0..1) as correct characters / total sent characters
 */
export function calculateOverallCharacterAccuracy(
  groups: ReadonlyArray<{ readonly sent: string; readonly received: string }>
): number {
  let totalSent = 0;
  let correctCount = 0;
  ...
  groups.forEach(group => {
    ...
    alignment.forEach(pair => {
      // Only count sent letters (ignore insertions - extra chars in received)
      if (pair.sentChar !== null) {
        totalSent++;
        if (pair.match) {
          correctCount++;
  ...
  return totalSent > 0 ? correctCount / totalSent : 0;
}
